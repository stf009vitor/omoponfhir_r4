...
import edu.gatech.chai.omoponfhir.omopv5.r4.mapping.OmopMedicationDispense;
...
public class MedicationDispenseResourceProvider implements IResourceProvider {
	...
	@Search()
	public IBundleProvider findMedicationDispensesById(
	...
	public IBundleProvider findMedicationDispensesByParams(
			...
			@OptionalParam(name = MedicationDispense.SP_RECEIVER) ReferenceParam theSource) {
		List<ParameterWrapper> paramList = new ArrayList<ParameterWrapper>();
...
//				} else {
		...
		if (theSource != null) {
			paramList.addAll(myMapper.mapParameter(MedicationDispense.SP_RECEIVER, theSource, false));
		}
